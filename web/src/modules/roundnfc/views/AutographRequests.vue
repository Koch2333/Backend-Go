...
import { listAutographRequests, setAutographStatus } from '../api'
import type { AutographRequest, RequestStatus } from '../types'

const items = ref<AutographRequest[]>([])
const total = ref(0)
...
async function load() {
  loading.value = true
  try {
    const res = await listAutographRequests({
...
async function update(r: AutographRequest, next: RequestStatus) {
  try {
    await setAutographStatus(r.id, next)
    r.status = next
  } catch (err) {
...
<template>
  <div class="space-y-5">
    <header class="m3-page-header">
      <div>
        <h1 class="m3-headline-medium text-on-surface">To 签申请</h1>
    ...
    <div v-else-if="items.length === 0" class="m3-card m3-empty">
      <div class="m3-empty-icon"><md-icon>edit_note</md-icon></div>
        ...
        <md-list-item>
          ...
          <div slot="supporting-text" class="row-supporting">
            <div class="m3-body-small text-on-surface-variant">
              {{ r.contact }} · {{ r.badgeId }} · 签在 {{ r.target || '—' }}
            </div>
            <div class="m3-body-medium text-on-surface">{{ r.content }}</div>
            <div class="m3-body-small text-on-surface-variant">
